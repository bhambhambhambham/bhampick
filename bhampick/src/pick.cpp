...
bool callback(bhampick::boxpose::Request &req, bhampick::boxpose::Response &res)
{
  ...
  createCollisionObjectFrame(broadcaster, "world", "box_frame", xx, yy, req.z, req.roll, req.pitch, req.yaw);
  ...
  srv.request.y = req.y;
  srv.request.z = req.z;
  srv.request.roll = req.roll;
  srv.request.pitch = req.pitch;
  srv.request.yaw = req.yaw;
  ...
  // ROS_INFO("Orientation : (%f, %f, %f)", proll, ppitch, pyaw);
  // pick(group, px, py, pz, proll, ppitch, pyaw, req.length, req.width, req.depth);
  // pick(group, req.x, req.y, req.z, proll, ppitch, pyaw, req.length, req.width, req.depth);

  float a = req.yaw;
  float b = tau/4;
  pyaw = fmod(a, b);
  pick(group, req.x, req.y, req.z, req.roll, req.pitch, pyaw, req.length, req.width, req.depth);
  return true;
}
